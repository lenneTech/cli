...
| Caddy integration | `src/lib/caddy.ts` | One block per project, marked with `# >>> lt-dev:<slug> >>>`/`# <<<`. `upsertProjectBlock` is idempotent; `removeProjectBlock` is a no-op when absent. Caddyfile path overridable via `LT_DEV_CADDYFILE`. The daemon is owned by `lt dev install` (see `dev-service.ts`) — **never** rely on `brew services caddy`: its plist hardcodes `--config /opt/homebrew/etc/Caddyfile` and crash-loops against our location, which is the bug that originally blocked the first real `lt dev install`. **Never touches a Caddy it did not start:** `detectCaddyOwner` decides by the LOADED config, never by process (a command-line check would be untested on every platform but one). It returns `ours` when the inert `log lt-dev-owner { output discard }` global option is loaded (comments do not survive adaptation, so the block markers cannot serve), or, for pre-marker instances, when the loaded config equals `caddy adapt` of our file; that is why callers ask BEFORE rewriting the Caddyfile. It returns `foreign` for anything else that answers on :2019, and `none` when nothing answers. Every command goes through `dev-caddy-gate.ts#ensureOwnCaddy`. A foreign Caddy gets `foreignCaddyLines` (our path + `caddy reload`/`caddy run`), never a stop or a reload. `__tests__/caddy-owner.test.ts` fails when a file calls `reloadCaddy`/`stopCaddy` without asking. |
...
### Starting the compiled API skips what the `start` script chains <!-- Added: 2026-09-21 -->
lt projects start with `migrate:up && start:local`. Any path that spawns the compiled bundle
directly skips that script, and the migration goes with it. `lt dev up --api-compiled` had
the step, but `lt dev test` never did. Its `<…>-test` DB is never dropped by the CLI, so it
fell further behind with every migration: a unique index made partial by a migration never
reached it, and the Playwright suite failed with a 409 that looked like a bug in the feature
(DEV-3289). A data migration is worse, because nothing fails at all. The bring-up log made
the diagnosis harder by claiming "reset before the suite by Playwright global-setup", which
the starter's global-setup does not do. **Rules:** every compiled start goes through
`applyPendingMigrations` with the env of the API itself, because the migration store resolves
its DB from the same `config.env.ts` + `NSC__MONGOOSE__URI` merge. A log line states only what
the CLI itself does. Behaviour is pinned in `__tests__/dev-test-session-api.test.ts`.
...
a project generated by lt 1.47.0 could not run `pnpm run start:local` on Windows —
cmd.exe reads `NODE_ENV=develop node …` as a command named `NODE_ENV`.
**Rules:** wrap env-carrying commands with `withCrossEnv` (`src/lib/cross-env.ts`) and call
`ensureCrossEnvDependency(pkg)` in the same place. It pins `cross-env` exactly, into
`dependencies` (see below), and only when the project does not already declare it — the
starters ship it themselves, and a second entry would fight theirs on every update; an
existing entry keeps its group and version. Use `|| exit 0`, never `|| true`.
`__tests__/generated-scripts-portable.test.ts` reads every `scripts[…] = …` assignment in
`src/**` from the TypeScript AST (template literals included, `${…}` → `X`) and checks
those rules, so a new generator is covered without touching the test.
**Two traps:**
- **The detector must not be anchored to the start of the script.** The mistake that
  happens when converting is an `&&` chain with only the first link wrapped
  (`cross-env NODE_ENV=x a && NODE_ENV=x b`); an anchored expression calls that clean.
  `bareEnvPrefixes` strips the assignments `cross-env` carries and then scans the rest,
  so `cross-env A=1 B=2 cmd` passes and the half-converted chain is reported. The repo's
  own guard routes through the same function — a plain regex there forbids the very fix
  its message recommends.
- **In a generated project `cross-env` belongs in `dependencies`.** Production trees are
  installed without dev packages (the starter's Dockerfile: `pnpm install --frozen-lockfile
  --prod`, then `pnpm deploy --prod`), and the scripts that run there — `start:prod`,
  `dp:prod` → `migrate:prod:up` — are the env-carrying ones. Measured on two real `--prod`
  installs from one lockfile: as a devDependency `node_modules/.bin/cross-env` is missing.
  (`docker-entrypoint.sh` calls the migrate binary directly, so the container start itself
  is unaffected.) The framework repos are the opposite case and keep it in devDependencies:
  their scripts never reach a customer image, and nest-server is published to npm, where a
