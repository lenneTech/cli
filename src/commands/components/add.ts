...
async function processConfig(config: any, toolbox: ExtendedGluegunToolbox) {
  ...
  if (config?.npm) {
    ...
    for (const npmPackage of npmPackages) {
      const confirm = await prompt.confirm(`The npm package ${npmPackage} is required. Would you like to install it?`);
  ...
  if (config?.components) {
    ...
    for (const component of components) {
      const confirm = await prompt.confirm(`The component ${component} is required. Would you like to add it?`);
