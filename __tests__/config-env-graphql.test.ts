import {
  ...
  isConfigEnvFile,
} from '../src/lib/config-env-graphql';
...
describe('isConfigEnvFile', () => {
  it('matches whatever separator the host produced', () => {
    // `filesystem.find` returns `pathUtil.relative(cwd, path)` — backslashes on
    // Windows. The previous `endsWith('/config.env.ts')` matched nothing there, so
    // the graphql region was stripped without its `graphQl: false` replacement.
    expect(isConfigEnvFile('../../src/config.env.ts')).toBe(true);
    expect(isConfigEnvFile('..\\..\\src\\config.env.ts')).toBe(true);
    expect(isConfigEnvFile('config.env.ts')).toBe(true);
    expect(isConfigEnvFile('src/config.env.example.ts')).toBe(false);
    expect(isConfigEnvFile('src\\server\\server.module.ts')).toBe(false);
  });
});
